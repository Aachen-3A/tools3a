...
def jdlreplace(value, cluster,process):
    #replace cluster and process
    value=re.sub(r"(?i)\$\(cluster\)",cluster,value)
    value=re.sub(r"(?i)\$\(process\)",process,value)
    return value
